...
if __name__ == "__main__":
    ...
    logger.info("Current directory: %s", here)

    with open(here.joinpath("urls.txt"), "r", encoding="utf-8") as infile:
        urls_to_crawl = set(map(str.strip, infile))

    logger.info("Crawling URLs: %s", urls_to_crawl)

    outpath = here.joinpath("foundurls.txt")
    with open(outpath, "w", encoding="utf-8") as outfile:
        outfile.write("source_url\tparsed_url\n")

    asyncio.run(bulk_crawl_and_write(file=outpath, urls=urls_to_crawl))
